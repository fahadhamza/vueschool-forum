...
const data = reactive({ posts: sourceData.posts, threads: sourceData.threads });
const newPostText = reactive({ text: "" });
...
const thread = computed({
  ...
  set(postId) {
    data.threads.find((thread) => thread.id === props.id).posts.push(postId);
  },
});
...
function addPost() {
  const postId = "vue" + Math.random();
  const post = {
    id: postId,
    publishedAt: Math.floor(Date.now() / 1000),
    text: newPostText.text,
    threadId: props.id,
